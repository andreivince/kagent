...
// controller's discovery probe dial an identical endpoint. It is gated by an
// OSS feature flag and used in two places: the agent translator calls
...
// computed) when the OSS egress gate is enabled — registration is the gate,
...
// before BuildManifest runs config-phase plugins — so by the time this sees
...
// k8s API fields like ServiceEntry.Ports.Number and Backend.Static.Port expect
// after casting). An explicit port in spec.url wins; otherwise the default is
